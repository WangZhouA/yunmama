...
public class WheelView extends View {
    ...
    public int getTextWidthOffSet() {
        return textWidthOffSet;
    }

    public void setTextWidthOffSet(int textWidthOffSet) {
        this.textWidthOffSet = textWidthOffSet;
    }

    private int textWidthOffSet = 10;//单位距离中心值的距离
...
//            int drawRightContentStart = measuredWidth - getTextWidth(paintCenterText, label);
            while (counter < itemsVisible) {//拿到某一个值的长度
                if (!visibles[counter].equals("")){
                    break;
                }
                counter ++;
            }
            //System.out.println("单位长度X = "+(getTextWidth(paintCenterText,getContentText(visibles[counter]))+getTextWidth(paintCenterText, label)*2 - CENTER_CONTENT_OFFSET));
            //距离位置=当前显示的值的长度+和单位的长度+间距（可以调整距离）
            canvas.drawText(label, drawCenterContentStart+
                    getTextWidth(paintCenterText,getContentText(visibles[counter]))+textWidthOffSet,
